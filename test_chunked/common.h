...
int receive_from_peer(peer_t *peer, int (*message_handler)(message_t *))
{
  ...
  std::cout << (char*)&peer->receiving_buffer << std::endl;
  return 0;
}
...
int send_to_peer(peer_t *peer)
{
  ...
  do {
    ...
    if (peer->current_sending_byte < 0 || peer->current_sending_byte >= strlen((char*)&peer->sending_buffer)) {
      ...
      std::cout << "buff : " << (char*)&peer->sending_buffer << std::endl;
      peer->current_sending_byte = 0;
      std::cout << "size : " << strlen((char*)&peer->sending_buffer) - peer->current_sending_byte <<std::endl;
    }
    
    // Count bytes to send.
    len_to_send = strlen((char*)&peer->sending_buffer) - peer->current_sending_byte;
