...
<head>
    <title>42-WEBSERV Effacement</title>
    <meta charset="UTF-8">
    ...
    function makeDELETErequest(fichier) {
        $.ajax({
            ...
            success: function (result) {
                alert(fichier+" effacé");
                location.reload(true);
            },
            error: function(result) {             
                alert("Impossible d'effacer "+fichier);
            }
        });
...
<body>
    ...
    foreach($scandir as $fichier){
      if(preg_match("#\.(jpg|jpeg|png|gif|bmp|tif|pdf|txt)$#",strtolower($fichier))){
            echo "<tr><th>$fichier</th>"."<td><button onclick=\"makeDELETErequest('".$fichier."')\">Effacer</button></td></tr>";
      }
    }
    ?>
        </tbody>
